...
function read(url, day, callback) {
  request(url, (err, res, body) => {
    ...
    timeString = [];
    //
    if (time[day].includes(undefined)) return callback(null, "Сегодня пар нет");
    for (let i = 0; i < 8; i++) {
      time[day][i].forEach(_class => {
        let str = "";
        str += "<b>" + classTimes[i] + "</b>\n";
        if (_class.parity.length > 0) str += _class.parity + "\n";
        if (_class.type.length > 0) str += _class.type + "\n";
        if (_class.subgroup.length > 0) str += _class.subgroup + "\n";
        if (_class.name.length > 0) str += _class.name + "\n";
        if (_class.professor.length > 0) str += _class.professor + "\n";
        if (_class.classroom.length > 0) str += _class.classroom + "\n";
