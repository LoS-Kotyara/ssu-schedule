...
function get(lastName, callback) {
    Nightmare({
        ...
        .goto(url)
        .wait("body")
        .type("#schedule_page > div > div.panes_item.panes_item__type_teacher > div.teacher-form > input[type=text]:nth-child(1)", lastName)
        .click("#schedule_page > div > div.panes_item.panes_item__type_teacher > div.teacher-form > input[type=button]:nth-child(2)")
        .wait(1500)
...
const professor = new WizardScene(
    "professor",
    (ctx) => {
        ctx.reply("Введите фамилию преподавателя", Markup.removeKeyboard(true).oneTime().resize().extra());
    ...
    (ctx) => {
        ...
        get(ctx.wizard.state.lastName, async (err, res) => {
            ...
            if (names.length == 0) {
                await ctx.reply("Поиск не дал результатов" +
                    "\nПожалуйста, введите фамилию заново", Markup.removeKeyboard(true).oneTime().resize().extra());
                ctx.reply("Введите фамилию преподавателя");
                return ctx.wizard.selectStep(1);
            }
            if (names.length > 15) {
                await ctx.reply("Поиск дал большое число результатов (" + names.length + ")" +
                    "\nПожалуйста, уточните фамилию преподавателя", Markup.removeKeyboard(true).oneTime().resize().extra());
                ctx.reply("Введите фамилию преподавателя");
                return ctx.wizard.selectStep(1);
            }
            ctx.reply("Выберите преподавателя", Markup.keyboard(names).resize().extra());
            return ctx.wizard.next();
        });
    },
    async (ctx) => {
        ...
        await ctx.reply('Вы выбрали "' + ctx.message.text + '"', Markup.removeKeyboard(true).oneTime().resize().extra());
        ...
        setTimeout(() => {
            ctx.reply("Выбрать другого преподавателя?", Markup.keyboard(['Да', 'Нет']).oneTime().resize().extra());
        }, 10000);
