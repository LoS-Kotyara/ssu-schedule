...
const {
  ...
  readCourses,
  readGroups
} = require("../helpers/getFacultyData");
...
const student = new WizardScene(
  ...
  async ctx => {
    ...
    ctx.reply(
      "Выберите факультет",
      Markup.keyboard(ctx.wizard.state.faculties)
        .resize()
        .extra()
    );
  ...
  async ctx => {
    ...
    if (ctx.wizard.state.faculties.indexOf(ctx.wizard.state.faculty) == -1) {
      ctx.reply(
        "Вы ввели неверное значение, выход из меню выбора",
    ...
    ctx.wizard.state.forms = await new readFacultyForms(
      ctx.wizard.state.faculty
    );
    ctx.wizard.state.forms.push("Назад");
    ctx.reply(
      "Выберите форму обучения",
      Markup.keyboard(ctx.wizard.state.forms)
        .resize()
        .extra()
    );
  ...
  async ctx => {
    ...
    ctx.wizard.state.types = await new readFormGroupTypes(
      ctx.wizard.state.faculty,
    ...
    ctx.reply(
      ...
      Markup.keyboard(ctx.wizard.state.types)
        .resize()
        .extra()
    );
  ...
  async ctx => {
    ...
    ctx.wizard.state.courses = await new readCourses(
      ctx.wizard.state.faculty,
      ctx.wizard.state.form,
    ...
    ctx.reply(
      "Выберите курс",
      Markup.keyboard(ctx.wizard.state.courses)
        .resize()
        .extra()
    );
  ...
  async ctx => {
    ...
    ctx.wizard.state.temp = await new readGroups(
      ctx.wizard.state.faculty,
      ctx.wizard.state.form,
      ctx.wizard.state.type,
      ctx.wizard.state.course
    );
    ...
    ctx.reply(
      "Выберите группу",
      Markup.keyboard(ctx.wizard.state.groups)
        .resize()
        .extra()
    );
  ...
  async ctx => {
    ...
    await ctx.reply(
      'Вы выбрали"' + ctx.wizard.state.group + '"',
      Markup.removeKeyboard(true)
        .oneTime()
        .resize()
        .extra()
    ...
    ctx.reply(
      "Вам нужно расписание на сегодня или на конкретный день?",
      Markup.keyboard(["На сегодня", "На конкретный день", "Назад"])
        .resize()
        .extra()
    );
  ...
  async ctx => {
    ...
    if (
      ctx.wizard.state.dayAns !== "На сегодня" &&
      ctx.wizard.state.dayAns !== "На конкретный день"
    ) {
      ctx.reply(
        "Вы ввели неверное значение, выход из меню выбора",
        Markup.removeKeyboard(true)
    ...
    let weekday = null;
    await ctx.reply(
      'Вы выбрали"' + ctx.wizard.state.dayAns + '"',
      Markup.removeKeyboard(true)
        .oneTime()
        .resize()
        .extra()
    );
    if (ctx.wizard.state.dayAns == "На сегодня") {
      weekday = weekdays[date.getDay()][0];
    } else {
      ctx.wizard.state.variables = weekdays.slice(1).concat([["Назад"]]);

      ctx.reply(
        "На какой день недели Вам нужно расписание",
        Markup.keyboard(ctx.wizard.state.variables)
          .resize()
          .extra()
      );
    ...
    if (weekday !== "Воскресенье") {
      ...
      await read(
        ctx.wizard.state.paths[
          ctx.wizard.state.groups.indexOf(ctx.wizard.state.group)
        ],
        weekday,
        (err, res) => {
  ...
  async ctx => {
    ...
    await ctx.reply(
      'Вы выбрали"' + ctx.wizard.state.weekday + '"',
      Markup.removeKeyboard(true)
        .oneTime()
        .resize()
        .extra()
